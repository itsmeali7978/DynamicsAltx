...
async function loadExistingBid(bidNo) {
    try {
        const response = await fetch(`http://192.168.105.238:8000/api/Bidding/BidDetails/${bidNo}`);
...
document.getElementById('btn-mark-inactive').addEventListener('click', async () => {
    ...
    try {
        const response = await fetch('http://192.168.105.238:8000/api/Bidding/UpdateBidStatus', {
            method: 'POST',
...
document.getElementById('btn-new-bid').addEventListener('click', async () => {
    ...
    try {
        const response = await fetch('http://192.168.105.238:8000/api/Bidding/InitializeBid', {
            method: 'POST'
        });
...
document.getElementById('btn-fetch-data').addEventListener('click', async () => {
    ...
    try {
        const response = await fetch('http://192.168.105.238:8000/api/Bidding/FetchAndSyncLines', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                bidNo: currentBidNo,
...
async function updateVendorPrice(lineId, vendorId, inputElement) {
    ...
    try {
        const response = await fetch('http://192.168.105.238:8000/api/Bidding/UpdateVendorPrice', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
